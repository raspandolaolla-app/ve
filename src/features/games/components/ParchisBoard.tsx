// ==============================================================================
// RASPANDO LA OLLA — TABLERO DE JUEGO: PARCHÍS / LUDO MULTIJUGADOR
// ==============================================================================
// Componente SVG altamente interactivo, responsivo y adaptativo para Parchís.
...
import type { ParchisState, ParchisColor, ParchisPiece } from '../../../types/games';
import type { TablePlayer } from '../../../types/tables';
import { BOARD_CONFIG_4, BOARD_CONFIG_6 } from '../engines/ParchisEngine';

export interface ParchisBoardProps {
  state: ParchisState;
...
const COLOR_MAP: Record<ParchisColor, { bg: string; fill: string; border: string; text: string; hex: string; ring: string }> = {
...
export const ParchisBoard: React.FC<ParchisBoardProps> = ({
  ...
  const [chatMessages, setChatMessages] = useState<ChatMessage[]>([
    { id: 'welcome', senderName: 'SISTEMA', text: '¡Bienvenidos a PARCHÍS VENEZOLANO! 🇻🇪', timestamp: 'Ahora', isSystem: true },
  ]);
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      ...
      if (remaining === 0 && !timeoutFiredRef.current && state.status === 'playing') {
  ...
  }, [state.turnDeadlineAt, turnExpiresAt, isMyTurn, state.status, onTimeout, onOpponentTimeout]);
...
// ==============================================================================
// TABLERO SVG COMPLETO DE PARCHÍS VENEZOLANO
// ==============================================================================
interface ParchisSVGProps {
  state: ParchisState;
  currentUserId: string;
...
const ParchisSVG: React.FC<ParchisSVGProps> = ({
  state,
  currentUserId,
  legalPieceIds,
  onMovePiece,
  ...
  return (
    <svg viewBox="0 0 500 500" className="w-full h-full select-none" id="parchis-svg-canvas">
      ...
      {(Object.values(state.pieces) as ParchisPiece[]).map((piece) => {
...
function getPieceSVGCoordinates(piece: ParchisPiece): { x: number; y: number } {
  // 1. Casa / Home Base
