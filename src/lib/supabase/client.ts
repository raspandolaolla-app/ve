...
// Obtención de credenciales públicas exclusivamente desde variables de entorno VITE_
// REGLA ABSOLUTA: No se permiten fallbacks hardcodeados ni lectura de SERVICE_ROLE_KEY
const rawUrl = (env.VITE_SUPABASE_URL || env.NEXT_PUBLIC_SUPABASE_URL || '') as string;
const rawAnonKey = (env.VITE_SUPABASE_ANON_KEY || env.NEXT_PUBLIC_SUPABASE_ANON_KEY || env.NEXT_PUBLIC_SUPABASE_PUBLISHABLE_KEY || '') as string;

// Validación controlada de presencia de credenciales públicas sin exponer secretos
if ((!rawUrl || !rawAnonKey) && typeof window !== 'undefined') {
  console.warn('[Supabase Client] Variables de entorno VITE_SUPABASE_URL y/o VITE_SUPABASE_ANON_KEY no detectadas. La conexión permanece deshabilitada de forma segura.');
}
