...
export class GameRepository {
  ...
  public static async submitRPSChoice(
    ...
    // 1. Invocar RPC autoritativa segura de commit-reveal en Supabase
    ...
    if (rpcError) {
      console.error('[GameRepository] submit_rps_choice_secure:', rpcError.message);
      throw new Error(rpcError.message || 'Error al registrar jugada en el servidor');
    }

    if (rpcData && !rpcData.success) {
      console.warn('[GameRepository] submit_rps_choice_secure rechazado:', rpcData.error || rpcData.message);
      throw new Error(rpcData.message || rpcData.error || 'Jugada no aceptada por el servidor');
    }

    return Boolean(rpcData?.success);
  }
  ...
  public static async nextRPSRound(
    sessionId: string,
    _fallbackState?: Record<string, unknown>
  ): Promise<boolean> {
    const supabase = getSupabaseClient();
    if (!supabase) return false;

    const { data: rpcData, error: rpcError } = await supabase.rpc('next_rps_round_secure', {
      p_session_id: sessionId,
    });

    if (rpcError) {
      console.error('[GameRepository] next_rps_round_secure:', rpcError.message);
      throw new Error(rpcError.message || 'Error al avanzar de ronda en el servidor');
    }

    if (rpcData && !rpcData.success) {
      console.warn('[GameRepository] next_rps_round_secure rechazado:', rpcData.error || rpcData.message);
      throw new Error(rpcData.message || rpcData.error || 'No se pudo avanzar de ronda');
    }
