    ...
    try {
      ...
      if (error || data?.success === false) {
        return {
          ...
          winnerUserId: data?.winner_user_id,
          error: data?.error || error?.message || 'Canto de Bingo no válido.',
        };
      }
    } catch (rpcErr: any) {
      console.warn('[TableRepository] RPC rpc_claim_bingo_secure falló:', rpcErr);
      return {
        success: false,
        error: rpcErr?.message || 'Error al validar canto de Bingo en el servidor.',
      };
    }

    return {
      success: false,
      error: 'No fue posible validar el canto de Bingo.',
    };
  }
