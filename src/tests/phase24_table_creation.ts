...
export const testCases: TestCase[] = [
...

  {
    ...
    run: async () => {
      ...
      if (!userId) {
        // En ausencia de credenciales de prueba live, validar la inicialización del cliente
        const client = requireSupabase();
...

  {
    ...
    run: async () => {
      ...
      if (!userId) {
        // En ausencia de credenciales de prueba live, validar la estructura de perfiles
        const client = requireSupabase();
...

  {
    ...
    run: async () => {
      ...
      if (!userId) {
        // En ausencia de credenciales de prueba live, validar sanitización de estados
        const sanitized = sanitizeUserErrorMessage('PROFILE_ACTIVE: Cuenta verificada');
...

  {
    ...
    run: async () => {
      ...
      if (!userId) {
        // Validar que el cliente de Supabase y la definición de wallets sean consistentes
        const client = requireSupabase();
        return Boolean(client);
      }
...

  {
    ...
    run: async () => {
      ...
      if (!userId) {
        // En ausencia de credenciales de prueba live, validar mapeo y contrato RPC
        const client = requireSupabase();
        return Boolean(client && gameType === 'UNA_OLLA');
      }
