-- ==============================================================================
-- MIGRACIÓN 066: REGISTRO E INTEGRACIÓN DEFINITIVA DE UNA_OLLA EN GAME_TYPE_ENUM Y RPCS
-- Proyecto: RASPANDO LA OLLA / PulsoPLAY
-- Estado: PRODUCCIÓN / SERVIDOR AUTORITATIVO IDEMPOTENTE
-- ==============================================================================

-- 1. Asegurar la presencia del enum 'UNA_OLLA' en public.game_type_enum
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_enum 
    WHERE enumtypid = 'game_type_enum'::regtype AND enumlabel = 'UNA_OLLA'
  ) THEN
    ALTER TYPE public.game_type_enum ADD VALUE 'UNA_OLLA';
  END IF;
END $$;

-- 2. Actualizar fn_normalize_game_type_enum para dar soporte completo a UNA_OLLA y sus alias
