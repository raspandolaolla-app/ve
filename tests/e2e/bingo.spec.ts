...
test.describe('Bingo Game Flow - E2E', () => {
  test('Debe interactuar con el tablero de Bingo, marcar números, ver avisos de proximidad de victoria y cantar bingo', async ({ page }) => {
        ...
        body: JSON.stringify([
          {
            ...
            invite_code: 'BNG-75',
            join_code: 'BNG-75',
            host_user_id: '7ef4010b-80a5-48b4-8ee1-d2a932d80dff',
    ...
    // Interceptar RPC de inicio de sesión de juego
    await page.route('**/rest/v1/rpc/start_game_session_secure', async (route) => {
      await route.fulfill({
        status: 200,
        contentType: 'application/json',
        body: JSON.stringify({ success: true, session_id: 'session-bingo-111' })
      });
    });

    // Interceptar RPC de cantar Bingo
    await page.route('**/rest/v1/rpc/rpc_claim_bingo_secure', async (route) => {
      await route.fulfill({
        status: 200,
        contentType: 'application/json',
        body: JSON.stringify({
          success: true,
          winner_user_id: '7ef4010b-80a5-48b4-8ee1-d2a932d80dff',
          winner_name: 'Robot Test',
          prize_bs: 90.0,
    ...
    // Capturar logs de la consola del navegador para diagnóstico
    page.on('console', msg => {
    ...
    // Hacer click en el botón de ingresar al juego dentro de la sala de espera
    const enterArenaBtn = page.locator('#btn-enter-game-arena');
    await expect(enterArenaBtn).toBeVisible({ timeout: 10000 });
